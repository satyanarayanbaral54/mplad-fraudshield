...
const amountLakhs = (project) => {
  const val = Number(project.allocated_amount || project.sanctioned_amount || 0);
  return val > 5000 ? val / 1e5 : val;
};
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[1180px] text-left text-xs border-collapse">
            <thead className="border-b border-slate-800 bg-slate-950 text-slate-400">
              <tr>
                <th className="p-4 align-middle">#</th>
                <th className="p-4 align-middle">{sortableHeader('title', 'Project Name')}</th>
                <th className="p-4 align-middle">{sortableHeader('mp_name', 'MP')}</th>
                <th className="p-4 align-middle">{sortableHeader('state', 'State')}</th>
                <th className="p-4 align-middle">Work Type</th>
                <th className="p-4 align-middle">{sortableHeader('amount', 'Amount (₹L)')}</th>
                <th className="p-4 align-middle">{sortableHeader('risk_score', 'Risk Score')}</th>
                <th className="p-4 align-middle">Risk Level</th>
                <th className="p-4 align-middle">Flags {detailLoading ? <span className="text-cyan-300">loading</span> : null}</th>
                <th className="p-4 text-right align-middle">Action</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-800/70 text-slate-300">
              {loading ? (
                ...
                pageProjects.map((project, index) => {
                  ...
                  return (
                    <tr key={project.id} className="h-16 transition hover:bg-white/[0.04]">
                      <td className="p-4 font-mono text-slate-500 align-middle">{(page - 1) * PAGE_SIZE + index + 1}</td>
                      <td className="max-w-[280px] p-4 font-bold text-white align-middle">
                        <Link to={`/projects/${project.id}`} className="line-clamp-2 leading-snug hover:text-cyan-300">{project.title}</Link>
                      </td>
                      <td className="p-4 align-middle whitespace-nowrap">{project.mp_name || 'N/A'}</td>
                      <td className="p-4 align-middle whitespace-nowrap">{project.state || 'N/A'}</td>
                      <td className="p-4 align-middle whitespace-nowrap">{getWorkType(project)}</td>
                      <td className="p-4 font-mono text-slate-200 align-middle whitespace-nowrap">₹{amountLakhs(project).toFixed(2)}</td>
                      ...
                      <td className="p-4 align-middle whitespace-nowrap"><RiskBadge level={project.risk_level} /></td>
                      <td className="max-w-[240px] p-4 align-middle">
                        {flags.length === 0 ? (
                          <span className="text-slate-500">None</span>
                        ) : (
                          <div className="flex flex-wrap items-center gap-1.5">
                            {flags.slice(0, 2).map((flag) => (
                              <span key={flag.id || flag.flag_type} className="rounded-full border border-white/10 bg-slate-900 px-2 py-0.5 text-[10px] font-semibold text-slate-200 whitespace-nowrap">
                                {flagIcon(flag.severity)} {flag.flag_type}
                              </span>
                            ))}
                            {flags.length > 2 ? <span className="rounded-full bg-slate-800 px-2 py-0.5 text-[10px] text-slate-400">+{flags.length - 2}</span> : null}
                          </div>
                        )}
                      </td>
                      <td className="p-4 text-right align-middle whitespace-nowrap">
                        <Link to={`/projects/${project.id}`} className="inline-flex h-9 items-center justify-center rounded-lg border border-cyan-400/25 bg-cyan-400/10 px-3.5 text-xs font-bold text-cyan-100 transition hover:border-cyan-300 hover:bg-cyan-400/20 shadow-sm">
                          View Details
                        </Link>
                      </td>
                    </tr>
