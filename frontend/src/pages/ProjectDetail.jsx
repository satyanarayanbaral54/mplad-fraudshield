...
const moneyLakhs = (value) => {
  const num = Number(value || 0);
  return num > 5000 ? num / 1e5 : num;
};

const actualRupees = (value) => {
  const num = Number(value || 0);
  return num > 5000 ? num : num * 1e5;
};
...
function MoneyBar({ label, value, max, color }) {
  const valLakhs = moneyLakhs(value);
  const maxLakhs = moneyLakhs(max);
  const percent = maxLakhs > 0 ? Math.min(100, (valLakhs / maxLakhs) * 100) : 0;
  return (
    <div>
      <div className="mb-1 flex items-center justify-between text-xs">
        <span className="font-semibold text-slate-300">{label}</span>
        <span className="font-mono text-slate-400">₹{valLakhs.toFixed(2)} L</span>
      </div>
...
export default function ProjectDetail() {
  ...
  const finances = useMemo(() => {
    const rawAllocated = project?.allocated_amount || 0;
    const rawSanctioned = getMeta(project, 'sanctioned_amount', project?.disbursed_amount || rawAllocated);
    const rawExpenditure = project?.expenditure || getMeta(project, 'expenditure_reported', 0);
    const rawUnspent = getMeta(project, 'unspent_balance', null);

    const allocated = moneyLakhs(rawAllocated);
    const sanctioned = moneyLakhs(rawSanctioned);
    const expenditure = moneyLakhs(rawExpenditure);
    const unspent = rawUnspent !== null ? moneyLakhs(rawUnspent) : Math.max(allocated - expenditure, 0);
          ...
          <table className="w-full border-collapse text-left text-xs">
            ...
            <tbody className="divide-y divide-slate-200">
              <tr>
                ...
                <td className="p-2 text-right font-mono font-bold">₹{finances.sanctioned.toFixed(2)}L</td>
                <td className="p-2 text-right font-mono">₹{Math.round(actualRupees(finances.sanctioned)).toLocaleString('en-IN')}</td>
              ...
              <tr>
                ...
                <td className="p-2 text-right font-mono font-bold">₹{finances.allocated.toFixed(2)}L</td>
                <td className="p-2 text-right font-mono">₹{Math.round(actualRupees(finances.allocated)).toLocaleString('en-IN')}</td>
              ...
              <tr>
                ...
                <td className="p-2 text-right font-mono font-bold">₹{finances.expenditure.toFixed(2)}L</td>
                <td className="p-2 text-right font-mono">₹{Math.round(actualRupees(finances.expenditure)).toLocaleString('en-IN')}</td>
              ...
              <tr className="bg-slate-50 font-bold">
                ...
                <td className="p-2 text-right font-mono">₹{finances.unspent.toFixed(2)}L</td>
                <td className="p-2 text-right font-mono">₹{Math.round(actualRupees(finances.unspent)).toLocaleString('en-IN')}</td>
